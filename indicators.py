...
def market_imbalance(df):
    ...
    for i in range(1, len(df)):
        ...
        if gap > atr(df).iloc[i] * 0.25:

            gaps.append(gap)

    largest = max(gaps) if gaps else 0

    nearest = gaps[-1] if gaps else 0
...
def volume_profile(df, bins=24):
    ...
    return {
        ...
        "hvn": safe_round(
            ...
            (prices[poc_index] + prices[poc_index + 1]) / 2

        ),

        "lvn": safe_round(

            (prices[lvn_index] + prices[lvn_index + 1]) / 2
...
# ==========================================================
# PROFESSIONAL COMPRESSION / EXPANSION
# ==========================================================
...
def compression_expansion(df):
    ...
    return {
        ...
        "compression": safe_round(current if compression else 0),

        "expansion": safe_round(current if expansion else 0),
...
def volume_events(df):
    ...
    highest_volume_price = df.loc[
        df["volume"].idxmax(),
        "close"
    ]

    lowest_volume_price = df.loc[
        df["volume"].idxmin(),
        "close"
    ]
    ...
    if avg50 > 0:
        ...
        relative = current_volume / avg50

    spike = current_volume > avg50 * 2

    dryup = current_volume < avg50 * 0.5
...
def candle_patterns(df):
    ...
    hammer = (
        lower_wick(df).iloc[-1] > body * 2
        and upper_wick(df).iloc[-1] < body
    )

    shooting = (
        upper_wick(df).iloc[-1] > body * 2
        and lower_wick(df).iloc[-1] < body
    )
